...
if ($search) {
    ...
    $where[] = "(full_name LIKE '%$search_sql%' OR email LIKE '%$search_sql%' OR subject LIKE '%$search_sql%' OR message LIKE '%$search_sql%')";
}
$where_sql = $where ? ('WHERE '.implode(' AND ', $where)) : '';
$totalRows = $conn->query("SELECT COUNT(*) FROM contact_messages $where_sql")->fetch_row()[0];
$totalPages = ceil($totalRows / $perPage);
$messages = $conn->query("SELECT * FROM contact_messages $where_sql ORDER BY submitted_at DESC LIMIT $perPage OFFSET $offset");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="main-content">
        ...
        <form class="table-controls" method="get" action="">
            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search name, email, subject, message...">
